...
# summary data frame of performance metrics
model.results <- data.frame(
  Model = c("Linear Regression", "kNN (k=15)", "CART"),
  MAE   = c(lm.MAE, knn.MAE, cart.MAE),
  RMSE  = c(lm.RMSE, knn.RMSE.opt, cart.RMSE),
  MAPE  = c(lm.MAPE, knn.MAPE, cart.MAPE),
  R2    = c(lm.R2, knn.R2, cart.R2)
)

print(model.results)

ggplot(model.results, aes(x = Model, y = RMSE, fill = Model)) +
  geom_bar(stat = "identity", width = 0.6) +
  geom_text(aes(label = round(RMSE, 1)), vjust = -0.5, size = 4) +
  labs(
    title = "Comparison of Model Errors across MLR, kNN, and CART",
    x = "Model",
...
scenarios <- scenarios %>%
  mutate(
    Change = Predicted_Revenue - lag(Predicted_Revenue, default = first(Predicted_Revenue)),
    Type = case_when(
...
# Compute start and end positions for each bar
scenarios <- scenarios %>%
  mutate(
    end = cumsum(Change) + first(Predicted_Revenue) - first(Change),
    start = lag(end, default = first(Predicted_Revenue)),
    label_pos = (start + end) / 2
  )

print(scenarios)

ggplot(scenarios, aes(x = Scenario, fill = Type)) +
  geom_rect(aes(
    xmin = as.numeric(Scenario) - 0.4,
    xmax = as.numeric(Scenario) + 0.4,
    ymin = pmin(start, end),
    ymax = pmax(start, end)
  )) +
  geom_text(aes(
    x = Scenario,
    y = label_pos,
    label = sprintf("%.2f", Predicted_Revenue)
  ),
  vjust = -0.5, size = 4) +
  
  # 👇 Include your color palette here
  scale_fill_manual(values = c(
    "Baseline" = "#bdbdbd",   # neutral gray
    "Increase" = "#4CAF50",   # green for positive change
    "Decrease" = "#E53935"    # red for negative change
  )) +
  
  labs(
    title = "Incremental Impact of Marketing and Discount Changes on Predicted Revenue",
    x = "",
    y = "Predicted Revenue"
  ) +
